...
import { prepareTicTacToe, prepareSigners } from "./utils/prepare"
    ...
    describe("2. Action", function () {
        ...
        describe("2.1 CreateGame", function () {
            ...
            it("2.1.1. Create one game", async function () {
                await expect(this.token4.connect(this.alice).createGame(this.timeWait[0])) // Создание игры
                    .to.emit(this.token4, 'CreateGame') // Тестирование события
                    .withArgs(0, this.alice.address, this.timeWait[0]) // Аргументы события
            ...
            it("2.1.2. Create multiple games", async function () {
                await expect(this.token4.connect(this.alice).createGame(this.timeWait[0])) // Создание первой игры
                    .to.emit(this.token4, 'CreateGame') // Тестирование события
                    .withArgs(0, this.alice.address, this.timeWait[0]) // Аргументы события
                this.result = await this.token4.getOneGame(0); // Получаем вторую игру и проверяем её параметры
                compareGame(this.result, this.alice.address, this.nullAddress, [0, 0, 0, 0, 0, 0, 0, 0, 0], BigNumber.from(0), this.timeWait[0], 0);

                await expect(this.token4.connect(this.alice).createGame(this.timeWait[1])) // Создание второй игры
                    .to.emit(this.token4, 'CreateGame') // Тестирование события
                    .withArgs(1, this.alice.address, this.timeWait[1]) // Аргументы события
                ...
                await expect(this.token4.connect(this.tema).createGame(this.timeWait[2])) // Создание третьей игры
                    .to.emit(this.token4, 'CreateGame') // Тестирование события
                    .withArgs(2, this.tema.address, this.timeWait[2]) // Аргументы события
                ...
                await expect(this.token4.connect(this.misha).createGame(this.timeWait[3])) // Создание четвертой игры
                    .to.emit(this.token4, 'CreateGame') // Тестирование события
                    .withArgs(3, this.misha.address, this.timeWait[3]) // Аргументы события
            ...
            it("2.1.3. Check require", async function () {
                await expect(this.token4.connect(this.alice).createGame(0)).to.be.reverted  // (Время ожидания игры должно быть > 0)
                await expect(this.token4.connect(this.alice).createGame(-10)).to.be.reverted  // (Время ожидания игры должно быть > 0)
            })
        })
        ...
        describe("2.2 PauseGame", function () {
            ...
            it("2.2.1. Pause/Continue game", async function () {
                await this.token4.connect(this.alice).createGame(this.timeWait[0]); // Создание тестовой игры

                await expect(this.token4.connect(this.alice).pauseGame(0)) // Ставим поиск игроков на паузу
                    .to.emit(this.token4, 'PauseGame') // Тестирование события 
                    .withArgs(0, 3) // Аргументы события
                ...
                await expect(this.token4.connect(this.alice).pauseGame(0)) // Возобновляем поиск игроков
                    .to.emit(this.token4, 'PauseGame') // Тестирование события 
                    .withArgs(0, 0) // Аргументы события
                this.result = await this.token4.getOneGame(0); // Получаем тестовую игру и проверяем её параметры
                compareGame(this.result, this.alice.address, this.nullAddress, [0, 0, 0, 0, 0, 0, 0, 0, 0], BigNumber.from(0), this.timeWait[0], 0);
            })
        ...
        describe("2.3 JoinGame", function () {
            ...
            it("2.3.1. Join one game", async function () {
                ...
                await expect(this.token4.connect(this.tema).joinGame(0)) // Присоединение к игре
                    .to.emit(this.token4, 'JoinGame') // Тестирование события 
                    .withArgs(0, this.tema.address) // Аргументы события
                this.result = await this.token4.getOneGame(0); // Получаем тестовую игру и проверяем её параметры
            ...
            it("2.3.2. Join multiple games", async function () {
                await this.token4.connect(this.alice).createGame(this.timeWait[0]); // Создание первой тестовой игры
                this.nowTime = await latest(); // Фиксируем время 
                ...
                await this.token4.connect(this.alice).createGame(this.timeWait[1]); // Создание второй тестовой игры
                this.nowTime = await latest(); // Фиксируем время 
                await expect(this.token4.connect(this.misha).joinGame(1)) // Присоединение к второй тестовой игре
                    .to.emit(this.token4, 'JoinGame') // Тестирование события 
                    .withArgs(1, this.misha.address) // Аргументы события
            ...
            it("2.3.4. Check require #2", async function () {
                ...
                await expect(this.token4.connect(this.misha).joinGame(0)) // Присоединение к тестовой игре
                    .to.emit(this.token4, 'JoinGame') // Тестирование события 
                    .withArgs(0, this.misha.address) // Аргументы события
        ...
        describe("2.4 MoveGame and IsFinish", function () {
            ...
            it("2.4.1. Turns player1 and player2", async function () {
                ...
                await expect(this.token4.connect(this.alice).movePlayer(0, this.moves1[0])) // Ход первого игрока
                    .to.emit(this.token4, 'MovePlayer') // Тестирование события 
                    .withArgs(0, this.alice.address, this.moves1[0]); // Аргументы события
                ...
                await expect(this.token4.connect(this.tema).movePlayer(0, this.moves1[1])) // Ход второго игрока
                    .to.emit(this.token4, 'MovePlayer') // Тестирование события 
                    .withArgs(0, this.tema.address, this.moves1[1]); // Аргументы события
                this.result = await this.token4.getOneGame(0); // Проверяем ход игры
                ...
                await expect(this.token4.connect(this.alice).movePlayer(0, this.moves1[2])) // Ход первого игрока
                    .to.emit(this.token4, 'MovePlayer') // Тестирование события 
                    .withArgs(0, this.alice.address, this.moves1[2]); // Аргументы события
                this.result = await this.token4.getOneGame(0); // Проверяем ход игры
                ...
                await expect(this.token4.connect(this.tema).movePlayer(0, this.moves1[3])) // Ход второго игрока
                    .to.emit(this.token4, 'MovePlayer') // Тестирование события 
                    .withArgs(0, this.tema.address, this.moves1[3]); // Аргументы события
                this.result = await this.token4.getOneGame(0); // Проверяем ход игры
            ...
            it("2.4.2. Player1 victory", async function () {
                ...
                this.result = await this.token4.getOneGame(0);
                compareGame(this.result, this.alice.address, this.tema.address, [2, 1, 0, 2, 1, 0, 0, 1, 0], this.nowTime, this.timeWait[0], 1);
            ...
            it("2.4.3. Player2 victory", async function () {
                ...
                this.result = await this.token4.getOneGame(0);
                compareGame(this.result, this.alice.address, this.tema.address, [2, 1, 0, 0, 2, 1, 1, 0, 2], this.nowTime, this.timeWait[0], 2);
            ...
            it("2.4.4. Draw", async function () {
                ...
                this.result = await this.token4.getOneGame(0);
                compareGame(this.result, this.alice.address, this.tema.address, [1, 2, 1, 2, 1, 1, 2, 1, 2], this.nowTime, this.timeWait[0], 1);
    ...
    describe("3. Getter", function () {
        ...
        describe("3.1 findOneGame", function () {
            it(" 3.1.1  Find games", async function () {
                await this.token4.connect(this.alice).createGame(this.timeWait[0]); // Создание тестовой игры
                await this.token4.connect(this.misha).createGame(this.timeWait[1]); // Создание тестовой игры
                await this.token4.connect(this.tema).createGame(this.timeWait[2]); // Создание тестовой игры
                await this.token4.connect(this.alice).createGame(this.timeWait[3]); // Создание тестовой игры
                await this.token4.connect(this.tema).createGame(this.timeWait[4]); // Создание тестовой игры
                this.answer = await this.token4.connect(this.bob).findOneGame(0, 0, 1000);  // Поиск игры, начиная с индекса 0
                expect(this.answer).to.eq(0);
                this.answer = await this.token4.connect(this.bob).findOneGame(2, 0, 1000); // Поиск игры, начиная с индекса 2
                expect(this.answer).to.eq(2);
                this.answer = await this.token4.connect(this.bob).findOneGame(0, 450, 1000); // Поиск игры, начиная с индекса 0 (Но с ограничением по времени)
            ...
            it(" 3.1.2 Check require", async function () {
                ...
                await this.token4.connect(this.alice).createGame(this.timeWait[3]); // Создание тестовой игры
                await this.token4.connect(this.tema).createGame(this.timeWait[4]); // Создание тестовой игры
                await expect(this.token4.connect(this.bob).findOneGame(5, 0, 1000)).to.be.reverted; //Индекс начала поиска больше чем количество доступных игр
                await expect(this.token4.connect(this.bob).findOneGame(0, 100, 50)).to.be.reverted; // Минимальное время больше максимального времени
                await expect(this.token4.connect(this.bob).findOneGame(0, 0, 50)).to.be.reverted;  // Не нашлось подходящих игр
            })
        })
        ...
        describe("3.2 findGamesPlayer", function () {
            it(" 3.2.1 Find all games by player", async function () {
                await this.token4.connect(this.alice).createGame(this.timeWait[0]); // Создание тестовой игры
                await this.token4.connect(this.misha).createGame(this.timeWait[1]); // Создание тестовой игры
                await this.token4.connect(this.alice).createGame(this.timeWait[2]); // Создание тестовой игры
                await this.token4.connect(this.alice).createGame(this.timeWait[3]); // Создание тестовой игры
        ...
        describe("3.3 Statistics", function () {
            it(" 3.3.1 Get statistics for player and  for games", async function () {
                // Создаем  15 игр
                for (let i = 0; i < 3; i++) {
                    await this.token4.connect(this.tema).createGame(this.timeWait[0]);
                    await this.token4.connect(this.misha).createGame(this.timeWait[0]);
                    await this.token4.connect(this.bob).createGame(this.timeWait[0]);
                    await this.token4.connect(this.alice).createGame(this.timeWait[0]);
                    await this.token4.connect(this.alice).createGame(this.timeWait[0]);
                }
